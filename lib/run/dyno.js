  ...
  _readData (c) {
    ...
    return data => {
      ...
      if (secondLine) {
        secondLine = false
        var p = data.toString()
        // cli.log(`Listening on ${cli.color.white.bold(port)} and forwarding to ${cli.color.white.bold(`${this.dyno.name}:${port}`)}`)
        child.fork(`heroku tunnels:port ${port} -a ${this.opts.app} > log.txt`)
        // port.forwardPort(this.heroku, p, p, this.opts.app, this.dyno.name)
        return
      }
      data = data.replace('\r\n', '\n')
